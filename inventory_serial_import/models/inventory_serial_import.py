...
class stock(models.Model):
    ...
    def read_validate_csv(self, csv_dict_reader):
        ...
        (which we know has no items with commas, so it cannot get screwed)
        matches the number of items in each row. If there is a mismatch,
        ...
        for row in csv_dict_reader:
            if len(row) != len(csv_dict_reader.fieldnames):
                raise csv.Error(
                    'Parsing failed: Header and row have different number \
                     of fields')
            # accumulate rows
            res.append(row)
        if not res:
    ...
    def read_lines(self, input_file):
        """Enumerate several most probable formats for csv.reader,
        first one is the same that is used for exporting stock picking objects.
        With each format, try to open the CSV-file
        ...
        formats = [
            # this format is used to export stock picking objects to CSV for
            # fulfillment
            # partners in stock_picking_out::ibood_csv_delivery()
            # (ibood_docdata/delivery.py)
            {'delimiter': ',', 'quoting': csv.QUOTE_MINIMAL,
             'escapechar': ';'},
            # this format may be used if the CSV is edited by tools
            # like LibreOffice
            ...
            {'delimiter': ',', 'quoting': csv.QUOTE_MINIMAL,
             'escapechar': '\\'}]
        # if the appropriate format will not be found afterward, save the error
        # text that will be obtained from parsing with the default
        # format at iteration 0.
        ...
        for f in formats:
            ...
            try:
                ...
                # check that each move line contains at least one value field
                for line in lines[:]:
                    for value in line.values():
                        ...
                        # if all fields are None or string consisting
                        # of spaces, then remove
        ...
        # appropriate format not found, raise exception with previously saved
        # error description
        ...
        if self.file_import:
            ...
            for rec in input_ids:
                if 'Serial Number' not in rec:
                    raise UserError(
                        'Row header "Serial Number" is not found in CSV file')
                ptype = self.picking_id.picking_type_id.code
                _logger.info("Picking type %s" % ptype)

                if self.picking_id.picking_type_id.code == "outgoing":

                    data = self.env['stock.production.lot'].search(
                        [('product_id', '=', self.product_id.id),
                         ('name', '=', rec.get('Serial Number'))])
                    data_list.append((0, 0, {'lot_id': data.id}))
                    if self.product_id != data.product_id:
                        raise UserError(
                            _('Serial Number %s does not belong to \
                               product - "%s".') % (rec.get('Serial Number'),
                                                    self.product_id.name))
                if self.picking_id.picking_type_id.code == "incoming":
                    data_list.append(
                        (0, 0, {'lot_name': rec.get('Serial Number')}))
            if self.picking_id.picking_type_id.code == "outgoing":
                if self.product_qty != len(input_ids):
                    raise UserError(
                        'Serial number count is greater than the product qty')
